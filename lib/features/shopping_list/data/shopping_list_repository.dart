/// Shopping list data access. The only place in this feature that touches
...
/// The columns of `shopping_lists` this feature reads.
const String _listColumns = '''
...
///
/// Narrower than `MealPlanRepository`'s own list: the shopping list never
/// renders a note's text or an entry's position, it only needs to know what
/// was planned, when, whether it was a leftover, and for how many people.
...
class ShoppingListRepository {
  const ShoppingListRepository(this._client);
  ...
  Future<ShoppingList?> fetchLatest({required String householdId}) =>
      runGuarded(() async {
        final Map<String, dynamic>? row = await _client
            .from('shopping_lists')
            .select('$_listColumns, shopping_list_items($_itemColumns)')
  ...
  ShoppingList _toList(Map<String, dynamic> row) {
    // PostgREST does not promise embed order, so sort by the column that
    // carries it -- the same thing fetchWeek and fetchDetail do for their own
    // children.
