/// Household data access to Supabase -- the Remote half of
/// `HouseholdRepository`'s split (Phase 2 part 7, D87-D90, on
/// `RemoteShoppingListDataSource`'s precedent, D64). The only place in this
/// feature that touches Supabase (CLAUDE.md rule 1).
///
/// This is the fourth Remote/Local split in the app, and the first one taken
/// where D70 had already ruled a split unnecessary for a similarly small
/// feature (`IngredientRepository`). The difference is what D87 is a report
/// about: `IngredientRepository.fetchUnitCatalog()`'s own read order has
/// never had a test, because there was never a seam to fake `SupabaseClient`
/// against. `fetchMineRows()` below is that seam for households -- see
/// `household_repository.dart`'s own doc comment for why an untested read
/// order was exactly the gap that let D87 through.
library;
...
class RemoteHouseholdDataSource {
  const RemoteHouseholdDataSource(this._client);

  final SupabaseClient _client;

  /// The raw rows of every household the caller belongs to, excluding
  ...
  ///
  /// `.retry(count: 1, requestTimeout: 5s)` is D89's fix for D87: nothing
  /// else in this app bounds a single request this tightly, because nothing
  /// else gates every household-scoped screen's own cache the way this call
  /// does. A global `Supabase.initialize(postgrestOptions:)` timeout was
  /// tried first and does not work -- verified against the pinned package
  /// source that `SupabaseClient.from()` forwards only `schema` from
  /// `PostgrestClientOptions`, never `requestTimeout` or `retryCount`, so a
  /// global option would never have reached this call at all (D89). Worst
  /// case here is ~11s (5s + 1s backoff + 5s) before
  /// `HouseholdRepository.fetchCurrent` falls back to cache, against the
  /// several minutes the emulator walk that found D87 actually measured.
  Future<List<Map<String, dynamic>>> fetchMineRows() =>
      runGuarded(() async {
        final List<Map<String, dynamic>> rows = await _client
            ...
            .order('created_at')
            .retry(count: 1, requestTimeout: const Duration(seconds: 5));
